...
DROPOUT_PROB = 0.2  # Increased dropout
VAL_SPLIT = 0.2
...
class ClothSegmentationDataset(Dataset):
    def __init__(self, img_dir, mask_dir, img_size, img_names, augment=True):
        ...
        # Enhanced normalization values specifically for clothing images
        self.transform = transforms.Compose([
            transforms.Resize(img_size),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])  # Better for binary clothing
        ])
    ...
    def __getitem__(self, idx):
        ...
        mask = (mask > 0.1).float()  # More sensitive threshold for dark clothing
