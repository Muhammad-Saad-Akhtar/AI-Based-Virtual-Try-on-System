...
NUM_EPOCHS = 75
LEARNING_RATE = 2e-4  # Increased learning rate to compensate for larger batch size
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
MODEL_SAVE_PATH = 'new_segmentation_unet.pth'
IMG_SIZE = (256, 192)  # Common aspect ratio for fashion images (4:3)
WEIGHT_DECAY = 1e-5
DROPOUT_PROB = 0.1
VAL_SPLIT = 0.2
...
# --- TRAINING ---
def train(model, dataloader, criterion, dice_criterion, boundary_criterion, optimizer, device):
    model.train()
    epoch_loss = 0
    for images, masks in tqdm(dataloader, desc="Training"):
        ...
        loss = 0.4 * bce_loss + 0.4 * dice_loss + 0.2 * boundary_loss
...
# --- VALIDATION ---
def validate(model, dataloader, criterion, dice_criterion, boundary_criterion, device):
    model.eval()
    epoch_loss = 0
    with torch.no_grad():
...
def main():
    ...
    val_img_names = [all_img_names[i] for i in val_indices]    train_dataset = ClothSegmentationDataset(IMG_DIR, MASK_DIR, IMG_SIZE, img_names=train_img_names, augment=True)
    ...
    if hasattr(torch, 'compile'):  # PyTorch 2.0 and later
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_loss = train(model, train_dataloader, criterion, dice_criterion, boundary_criterion, optimizer, DEVICE)
        val_loss = validate(model, val_dataloader, criterion, dice_criterion, boundary_criterion, DEVICE)
