...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        right_hip = landmarks[mp_pose.PoseLandmark.RIGHT_HIP]        shoulder_distance_horizontal = abs(left_shoulder.x - right_shoulder.x)
        frame_width = frame.shape[1]
        frame_height = frame.shape[0]
        
        # Calculate shirt width based on shoulder width with a smaller scaling factor
        shirt_width = int(shoulder_distance_horizontal * frame_width * 1.2)
        ...
        # Set shirt height based on torso height
        desired_height = torso_height * 1.1  # Slightly longer than torso
        aspect_ratio = shirt_no_bg.shape[0] / shirt_no_bg.shape[1]
        shirt_height = int(min(desired_height, shirt_width * aspect_ratio))

        shirt_resized = cv2.resize(shirt_no_bg, (shirt_width, shirt_height))
        mask_resized = cv2.resize(shirt_mask, (shirt_width, shirt_height))        # Calculate horizontal position (centered on shoulders)
        shoulder_center_x = (left_shoulder.x + right_shoulder.x) / 2
        shirt_x = int(shoulder_center_x * frame_width - shirt_width / 2)
        
        # Calculate vertical position (aligned with shoulders)
        shoulder_y_pixel = int(shoulder_y * frame_height)
        shirt_y = shoulder_y_pixel - int(shirt_height * 0.2)  # Place shirt slightly below shoulders
