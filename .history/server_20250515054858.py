...
class VirtualTryOn:
    ...
    def stop_camera(self):
        if self.cap is not None:
            self.cap.release()
            self.cap = None    def process_frame(self):
        if not self.cap or not self.shirt_no_bg:
            return None
        ...
        return framedef apply_virtual_tryon(self, frame, landmarks):
        ...
        aspect_ratio = self.shirt_no_bg.shape[0] / self.shirt_no_bg.shape[1]
        shirt_height = int(torso_height * 1.7)  # Increased from 1.3
        
        # Resize shirt and mask
        shirt_resized = cv2.resize(self.shirt_no_bg, (shirt_width, shirt_height))
        mask_resized = cv2.resize(self.shirt_mask, (shirt_width, shirt_height))
        ...
        try:
            ...
            # Ensure the mask has the same number of channels as the shirt region
            ...
            print(f"Error: Shirt region out of bounds: {e}")

        # Draw status bar with FPS
        fps = self.calculate_fps()
        frame = self.draw_status_bar(frame, fps)
        
        # Draw thumbnail
        frame = self.draw_thumbnail(frame)
    ...
    def calculate_fps(self):
        ...
        while self.fps_history and current_time - self.fps_history[0] > 1.0:
            self.fps_history.popleft()
        
        return len(self.fps_history)
    
    def draw_status_bar(self, frame, fps):
        # Extract filename without extension for display
        shirt_display = os.path.splitext(os.path.basename(self.shirt_name))[0] if self.shirt_name else "None"
    ...
    def draw_thumbnail(self, frame):
        if self.shirt_no_bg is None or self.shirt_mask is None:
            return frame
        ...
        thumb_shirt = cv2.resize(self.shirt_no_bg, (thumb_width, thumb_height))
        thumb_mask = cv2.resize(self.shirt_mask, (thumb_width, thumb_height))
