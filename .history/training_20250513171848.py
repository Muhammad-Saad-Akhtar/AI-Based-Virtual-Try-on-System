...
# --- PLOTTING ---
def plot_training_progress(train_losses, val_losses, save_path='training_progress.png', show_plot=True):
    # Only create new figure if showing plot
    if show_plot:
    ...
    # Add the current loss values to the plot
    if len(train_losses) > 0 and len(val_losses) > 0:
        current_train = train_losses[-1]
        current_val = val_losses[-1]
        plt.text(0.02, 0.95, f'Current Train Loss: {current_train:.4f}', 
                transform=plt.gca().transAxes, bbox=dict(facecolor='white', alpha=0.8))
        plt.text(0.02, 0.90, f'Current Val Loss: {current_val:.4f}', 
                transform=plt.gca().transAxes, bbox=dict(facecolor='white', alpha=0.8))
...
def main():
    ...
    # Plot every N epochs
    PLOT_INTERVAL = 5  # Only update plot every 5 epochs
