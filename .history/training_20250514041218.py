...
def main():
    ...
    test_dataset = ClothSegmentationDataset(TEST_IMG_DIR, TEST_MASK_DIR, IMG_SIZE,
                                             ...
                                             augment=False)
    test_dataloader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=4)    # Model, Loss, Optimizer    model = SegmentationModel().to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        val_loss = validate(model, val_dataloader, criterion, DEVICE)        print(f"Epoch {epoch + 1}/{NUM_EPOCHS}, Train Loss: {train_loss:.4f}, Validation Loss: {val_loss:.4f}")
