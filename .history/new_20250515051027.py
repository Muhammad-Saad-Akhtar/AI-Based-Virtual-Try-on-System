...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        # Calculate shirt width based on shoulder width (reduced scaling)
        shoulder_distance = abs(left_shoulder.x - right_shoulder.x)
        shirt_width = int(shoulder_distance * frame_width * 1.2)  # Reduced from 1.6
        ...
        # Set shirt height based on torso height while maintaining aspect ratio
        aspect_ratio = shirt_no_bg.shape[0] / shirt_no_bg.shape[1]
        shirt_height = int(torso_height * 1.1)  # Slightly longer than torso
