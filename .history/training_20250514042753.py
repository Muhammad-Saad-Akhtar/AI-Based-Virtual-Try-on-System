...
class EdgeAwareLoss(nn.Module):  
    ...
    def forward(self, pred, edge_pred, target):
        ...
        edge_loss = self.edge_loss(edge_pred)        # Calculate edge map from target for supervision
        target = target.float()  # Ensure target is float
        # Compute Sobel edges for each image in the batch using kornia filters
        sobel_grad = kf.spatial_gradient(target, normalized=True)  # Returns both x and y gradients
        sobel_x = sobel_grad[:, :, 0]  # x derivative
        sobel_y = sobel_grad[:, :, 1]  # y derivative
        target_edges = torch.sqrt(sobel_x.pow(2) + sobel_y.pow(2))

        # Ensure both have same dimensions for BCE loss
