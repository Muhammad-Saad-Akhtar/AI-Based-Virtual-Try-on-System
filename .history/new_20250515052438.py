...
def calculate_fps(fps_history):
    ...
    while fps_history and fps_history[0] < current_time - 1:
        fps_history.popleft()
    
    return len(fps_history)

# Add status bar drawing function
def draw_status_bar(frame, fps, shirt_name):
    ...
    # Add FPS text
    fps_text = f"FPS: {fps}"
    cv2.putText(status_bar, fps_text, (10, 20), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

    # Add active shirt name
    shirt_text = f"Active Shirt: {shirt_name}"
    cv2.putText(status_bar, shirt_text, (200, 20), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

    # Combine status bar with frame
    return np.vstack([status_bar, frame])

# Add thumbnail drawing function
def draw_thumbnail(frame, shirt_image, mask):
    # Create thumbnail of the shirt
    thumb_size = (100, 100)
    thumbnail = cv2.resize(shirt_image, thumb_size)
    thumb_mask = cv2.resize(mask, thumb_size)
    
    # Apply mask to thumbnail
    thumbnail = cv2.bitwise_and(thumbnail, thumbnail, mask=thumb_mask)
    
    # Create white background
    white_bg = np.ones((thumb_size[0], thumb_size[1], 3), dtype=np.uint8) * 255
    
    # Blend thumbnail with white background
    alpha = thumb_mask.astype(float) / 255
    alpha = np.expand_dims(alpha, axis=-1)
    thumbnail_with_bg = (thumbnail * alpha + white_bg * (1 - alpha)).astype(np.uint8)
    
    # Add border
    cv2.rectangle(frame, (10, 40), (110, 140), (0, 0, 0), 2)
    
    # Insert thumbnail into frame
    frame[40:140, 10:110] = thumbnail_with_bg
    return frame
